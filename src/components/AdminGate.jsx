    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <p className="text-red-400 font-mono text-xs mt-2 tracking-[0.2em] uppercase">Authorized Personnel Only</p>
        </div>
        ...
        <form onSubmit={handleUnlock} className="space-y-6">
          <div className="group relative">
            ...
            <input 
              ...
              className="w-full bg-black/50 border border-slate-700 rounded-xl px-12 py-4 text-white placeholder:text-slate-600 focus:outline-none focus:border-red-500 transition-all font-mono text-center tracking-[0.5em] text-lg"
              autoFocus
            />
          </div>
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-red-600 hover:bg-red-500 text-white font-bold uppercase tracking-widest py-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50"
          >
            {loading ? <Loader2 className="animate-spin" /> : <>Unlock Terminal <ChevronRight size={18} /></>}
          </button>
        </form>
      </motion.div>
    </div>
